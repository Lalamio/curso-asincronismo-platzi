...
function fetchData(urlApi, callback) {
  ...
  xhttp.onreadystatechange = function (event) {
    ...
    if(xhttp.readyState === 4) {
      ...
      if(xhttp.status >= 200 && xhttp.status < 300) {
        ...
        const error = new Error('Error' + 'status code: ' + xhttp.status + ' ' + urlApi);
  ...
  xhttp.send();
    ...
    //se invoca el metodo fetchData() pasándole como argumentos la variable API concatenada con la cadena 'products' para acceder a la URL de la API deseada, y una función anónima que recibe 2 parámetros (un objeto de error y un arreglo que almacena todos los objetos traídos por la API).
